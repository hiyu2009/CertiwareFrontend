System.register(['angular2/core', 'angular2/http'], function(exports_1, context_1) {
    "use strict";
    var __moduleName = context_1 && context_1.id;
    var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
        var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
        if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
        else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
        return c > 3 && r && Object.defineProperty(target, key, r), r;
    };
    var __metadata = (this && this.__metadata) || function (k, v) {
        if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
    };
    var core_1, http_1;
    var ProjectCodeService;
    return {
        setters:[
            function (core_1_1) {
                core_1 = core_1_1;
            },
            function (http_1_1) {
                http_1 = http_1_1;
            }],
        execute: function() {
            //프로젝트 목록보기시 조회시 선택하는 부서의 데이터를 관리
            ProjectCodeService = (function () {
                function ProjectCodeService(http) {
                    this.http = http;
                }
                ProjectCodeService.prototype.getCode = function () {
                    // let url="http://localhost:8080/project/selectCode";
                    // return this.http.get(url);
                    var url = "http://localhost:8080/project/selectCode";
                    var header = new http_1.Headers({ 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem("token") });
                    return this.http.post(url, null, { headers: header });
                };
                ProjectCodeService = __decorate([
                    core_1.Injectable(), 
                    __metadata('design:paramtypes', [(typeof (_a = typeof http_1.Http !== 'undefined' && http_1.Http) === 'function' && _a) || Object])
                ], ProjectCodeService);
                return ProjectCodeService;
                var _a;
            }());
            exports_1("ProjectCodeService", ProjectCodeService);
        }
    }
});
//# sourceMappingURL=projectCode.service.js.map